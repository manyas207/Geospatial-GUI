...
**Adding a new model?** See the full walkthrough: **[ADDING_A_MODEL.md](ADDING_A_MODEL.md)** (backend `ModelSpec`, tract zonal join, frontend plugin, testing).
...
Each analysis model has a **frontend plugin** under `web/plugins/` (contract in `web/model_plugin.js`). Plugins are registered in `web/dashboard_adapter.js` (ES module).

`dashboard_adapter.js` loads `GET /api/models` and merges API metadata with each plugin’s `presentation` object (labels, choropleth field, units, bar chart titles, Ask progress text).

| File | Role |
|------|------|
| `web/model_plugin.js` | Plugin factory (`createPlugin`) and hook defaults |
| `web/plugins/lst_plugin.js` | LST presentation, legend, tract popup, chat hints |
| `web/plugins/obia_plugin.js` | OBIA presentation and rendering hooks |
| `web/dashboard_adapter.js` | Plugin registry + merge with API model specs |
| `web/app.js` | Ask: model dropdown, two-step add/run, model-aware progress bar |
| `web/gf_frame.js` | Dashboard shell entry |
| `web/gf_frame_shared.js` | Shared state, adapter wiring, project load |
| `web/gf_frame_map.js` | MapLibre map, layers, legend |
| `web/gf_frame_chat.js` | Query panel, follow-up chat, PDF export |

### Presentation fields (Ask + dashboard)

Common keys in `plugin.presentation`:

| Key | Purpose |
|-----|---------|
| `runVerb` | Short model name in run button (e.g. `LST`, `OBIA`) |
| `runProgressStart` | Progress bar text when run starts |
| `runProgressWorking` | Progress bar text while status is `processing` |
| `choroplethField` | Tract attribute for map coloring |
| `primaryMetricKeys` | Keys in `run_stats` for charts and stats cards |
| `metricUnit` | Unit suffix (e.g. `°C`) |

When adding a model with dashboard type `equity`, create `web/plugins/your_model_plugin.js` and register it in `dashboard_adapter.js`. Models with `dashboard: "raster"` show a placeholder map shell until a dedicated view is built.

## Checklist: add model #3

See **[ADDING_A_MODEL.md](ADDING_A_MODEL.md)** for the complete guide. Short version:

1. Implement `models/your_model.py` with `run` and optional `post_process`.
2. Register in `models/registry.py`.
3. Add `web/plugins/your_model_plugin.js` and register in `web/dashboard_adapter.js`.
4. Smoke-test: Ask → add city → upload → run → dashboard → one chat question.
